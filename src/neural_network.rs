...
impl NeuralNetwork {
    ...
    pub fn with_layers_and_mode(
        ...
        for &layer_size in layers.iter().skip(1) {
            let layer_biases: Vec<f64> =
                (0..layer_size).map(|_| rng.gen_range(-1.0..1.0)).collect();
    ...
    pub fn with_hybrid_learning(
        layer_sizes: &[usize],
        hebbian_rate: f64,
        backprop_rate: f64,
    ) -> Self {
        let mut network =
            Self::with_layers_and_mode(layer_sizes, hebbian_rate, HebbianLearningMode::Hybrid);
        ...
        for layer_idx in 0..self.weights.len() {
            ...
            for to_neuron in 0..next_layer.len() {
                ...
                for from_neuron in 0..current_layer.len() {
                    sum += current_layer[from_neuron]
                        * self.weights[layer_idx][from_neuron][to_neuron];
                }
    ...
    fn apply_classic_hebbian(&mut self, activations: &[Vec<f64>]) {
        for layer_idx in 0..self.weights.len() {
            ...
            self.weights[layer_idx].par_iter_mut().enumerate().for_each(
                |(from_neuron, weight_row)| {
                    weight_row
                        .par_iter_mut()
                        .enumerate()
            ...
            if layer_idx > 0 {
                ...
                for from_neuron in 0..input_layer.len() {
                    ...
                    for to_neuron in 0..layer_activations.len() {
                        if to_neuron != winner {
                            let inhibition =
                                self.anti_hebbian_rate * input_layer[from_neuron] * 0.1;
                ...
                |(from_neuron, weight_row)| {
                    weight_row
                        ...
                        .for_each(|(to_neuron, weight)| {
                            ...
                            let weight_update = self.hebbian_rate
                                * post_activity
                ...
                |(from_neuron, weight_row)| {
                    weight_row
                        ...
                        .for_each(|(to_neuron, weight)| {
                            ...
                            let weight_update =
                                -self.anti_hebbian_rate * pre_activity * post_activity;
                            *weight += weight_update;
                        });
                },
            );
    ...
    fn apply_homeostatic_regulation(&mut self, activations: &[Vec<f64>]) {
        for layer_idx in 1..activations.len() {
            ...
            let _avg_activity: f64 =
                layer_activations.iter().sum::<f64>() / layer_activations.len() as f64;
    ...
    fn apply_backpropagation(&mut self, activations: &[Vec<f64>], targets: &[f64]) {
        ...
        for i in 0..self.layers[output_layer_idx] {
            ...
            layer_errors[output_layer_idx][i] =
                error * Self::sigmoid_derivative(activations[output_layer_idx][i]);
        }
            ...
            layer_errors[layer_idx] = (0..self.layers[layer_idx])
                ...
                .map(|neuron| {
                    let error: f64 = (0..self.layers[layer_idx + 1])
                        .into_par_iter()
                        .map(|next_neuron| {
                            layer_errors[layer_idx + 1][next_neuron]
                                * self.weights[layer_idx][neuron][next_neuron]
                        })
                        .sum();
    ...
    fn apply_hebbian_to_layer(&mut self, layer_idx: usize) {
        ...
        for from_neuron in 0..self.layers[from_layer] {
            for to_neuron in 0..self.layers[to_layer] {
                ...
                let correlation =
                    self.calculate_correlation(from_layer, from_neuron, to_layer, to_neuron);
    ...
    fn apply_online_hebbian_to_layer(&mut self, layer_idx: usize, activations: &[Vec<f64>]) {
        ...
        match self.learning_mode {
            ...
            HebbianLearningMode::Competitive => {
                self.apply_online_competitive_learning(
                    layer_idx,
                    from_layer,
                    to_layer,
                    online_rate,
            ...
            HebbianLearningMode::Hybrid => {
                ...
                self.apply_online_classic_hebbian(
                    layer_idx,
                    from_layer,
                    to_layer,
                    online_rate * 0.5,
    ...
    fn apply_online_classic_hebbian(
        &mut self,
        layer_idx: usize,
        from_layer: &[f64],
        to_layer: &[f64],
    ...
    fn apply_online_competitive_learning(
        &mut self,
        layer_idx: usize,
        from_layer: &[f64],
        to_layer: &[f64],
        ...
        for from_neuron in 0..from_layer.len() {
            ...
            let weight_update = rate
                * pre_activity
                * (post_activity - self.weights[layer_idx][from_neuron][winner]);
            self.weights[layer_idx][from_neuron][winner] += weight_update;
        }
    }

    /// Online Oja's rule with normalization
    fn apply_online_oja_rule(
        &mut self,
        layer_idx: usize,
        from_layer: &[f64],
        to_layer: &[f64],
        ...
        for from_neuron in 0..from_layer.len() {
            for to_neuron in 0..to_layer.len() {
                ...
                let weight_update =
                    rate * post_activity * (pre_activity - post_activity * current_weight);
                self.weights[layer_idx][from_neuron][to_neuron] += weight_update;
            }
        }
    }
...
mod tests {
    ...
    fn test_save_and_load_json() {
        ...
        // Load from file
        let mut loaded_nn =
            NeuralNetwork::load_from_file(filename).expect("Failed to load network");
        ...
        for (orig_layer, loaded_layer) in original_nn.weights.iter().zip(loaded_nn.weights.iter()) {
            for (orig_neuron, loaded_neuron) in orig_layer.iter().zip(loaded_layer.iter()) {
                for (orig_weight, loaded_weight) in orig_neuron.iter().zip(loaded_neuron.iter()) {
                    assert!(
                        (orig_weight - loaded_weight).abs() < 1e-10,
                        "Weight difference too large: {} vs {}",
                        orig_weight,
                        loaded_weight
                    );
                }
        ...
        for (orig_layer, loaded_layer) in original_nn.biases.iter().zip(loaded_nn.biases.iter()) {
            for (orig_bias, loaded_bias) in orig_layer.iter().zip(loaded_layer.iter()) {
                assert!(
                    (orig_bias - loaded_bias).abs() < 1e-10,
                    "Bias difference too large: {} vs {}",
                    orig_bias,
                    loaded_bias
                );
            }
    ...
    fn test_save_and_load_binary() {
        ...
        let mut original_nn =
            NeuralNetwork::with_layers_and_mode(&[3, 4, 2], 0.05, HebbianLearningMode::Oja);
        ...
        let mut loaded_nn =
            NeuralNetwork::load_from_binary(filename).expect("Failed to load network");
    ...
    fn test_export_metadata() {
        let nn = NeuralNetwork::with_layers_and_mode(&[2, 4, 3, 1], 0.03, HebbianLearningMode::BCM);
    ...
    fn test_serialization_with_online_learning() {
        ...
        let mut original_nn =
            NeuralNetwork::with_online_learning(&[2, 3, 1], 0.02, HebbianLearningMode::Hybrid);
        ...
        original_nn
            .save_to_file(filename)
            .expect("Failed to save online network");
        let mut loaded_nn =
            NeuralNetwork::load_from_file(filename).expect("Failed to load online network");

        // Verify online learning state is preserved
        assert_eq!(
            original_nn.is_online_learning(),
            loaded_nn.is_online_learning()
        );
