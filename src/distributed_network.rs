...
impl MessagePayload {
    ...
    fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            ...
            MessagePayload::HandshakeAck { network_id, accepted } => {
                buffer.extend_from_slice(network_id.as_bytes());
            ...
            MessagePayload::BackwardData { layer_id, gradients } => {
                buffer.push(*layer_id);
            ...
            MessagePayload::WeightSync { layer_id, weights, biases } => {
                buffer.push(*layer_id);
    ...
    fn from_bytes(msg_type: MessageType, bytes: &[u8]) -> Result<Self, ProtocolError> {
        ...
        match msg_type {
            MessageType::Handshake => {
                if bytes.len() < 16 { return Err(ProtocolError::InvalidPayload); }
                
                let network_id = Uuid::from_bytes_le(bytes[offset..offset+16].try_into().unwrap());
                offset += 16;
                
                let name_len = bytes[offset] as usize;
                offset += 1;
                
                if bytes.len() < offset + name_len { return Err(ProtocolError::InvalidPayload); }
                ...
                for _ in 0..layers_count {
                    if bytes.len() < offset + 2 { return Err(ProtocolError::InvalidPayload); }
                    let layer_size = BigEndian::read_u16(&bytes[offset..offset+2]);
                    layers.push(layer_size);
                    offset += 2;
                }
                
                if bytes.len() < offset + 4 { return Err(ProtocolError::InvalidPayload); }
                let capabilities = BigEndian::read_u32(&bytes[offset..offset+4]);
                
                Ok(MessagePayload::Handshake { network_id, name, layers, capabilities })
            }
            
            MessageType::ForwardData => {
                if bytes.len() < 5 { return Err(ProtocolError::InvalidPayload); }
                
                let layer_id = bytes[0];
                let data_len = BigEndian::read_u32(&bytes[1..5]) as usize;
                
                if bytes.len() != 5 + data_len * 4 { return Err(ProtocolError::InvalidPayload); }
            ...
            MessageType::HandshakeAck => {
                if bytes.len() != 17 { return Err(ProtocolError::InvalidPayload); }
                let network_id = Uuid::from_bytes_le(bytes[0..16].try_into().unwrap());
                let accepted = bytes[16] != 0;
                Ok(MessagePayload::HandshakeAck { network_id, accepted })
            }
            
            MessageType::Heartbeat => {
                if bytes.len() != 8 { return Err(ProtocolError::InvalidPayload); }
            ...
            MessageType::BackwardData => {
                ...
                Ok(MessagePayload::BackwardData { layer_id, gradients })
            }
            
            MessageType::HebbianData => {
                if bytes.len() < 9 { return Err(ProtocolError::InvalidPayload); }
                
                let layer_id = bytes[0];
                let learning_rate = f32::from_bits(BigEndian::read_u32(&bytes[1..5]));
                let data_len = BigEndian::read_u32(&bytes[5..9]) as usize;
                
                if bytes.len() != 9 + data_len * 4 { return Err(ProtocolError::InvalidPayload); }
                ...
                Ok(MessagePayload::HebbianData { layer_id, correlations, learning_rate })
            }
...
impl DistributedNetwork {
    ...
    async fn handle_connection(
        ...
        loop {
            ...
            stream.read_exact(&mut buffer[HEADER_SIZE..total_len]).await?;
    ...
    pub async fn connect_to(&self, address: &str, port: u16) -> Result<NetworkId, ProtocolError> {
        ...
        match NetworkMessage::from_bytes(&full_message) {
            Ok(ack_message) => {
                if let MessagePayload::HandshakeAck { network_id, accepted } = ack_message.payload {
                    if accepted {
    ...
    pub async fn send_forward_data(&self, peer_id: NetworkId, layer_id: u8, data: Vec<f64>) -> Result<(), ProtocolError> {
    ...
    pub async fn send_hebbian_data(&self, peer_id: NetworkId, layer_id: u8, correlations: Vec<f64>, learning_rate: f64) -> Result<(), ProtocolError> {
    ...
    async fn send_message_to_peer(&self, peer_id: NetworkId, message: NetworkMessage) -> Result<(), ProtocolError> {
        ...
        if let Some(_connection) = connections.get(&peer_id) {
            ...
            println!("📤 Would send {:?} message to {}", message.msg_type, peer_id);
        }
    ...
    pub async fn handle_message(&self, message: NetworkMessage) -> Result<(), ProtocolError> {
        match message.payload {
            MessagePayload::ForwardData { layer_id, data } => {
                println!("📥 Received forward data for layer {}: {} values", layer_id, data.len());
            ...
            MessagePayload::HebbianData { layer_id, correlations, learning_rate } => {
                println!("🧬 Received Hebbian data for layer {}: {} correlations (rate: {})", 
                        layer_id, correlations.len(), learning_rate);
