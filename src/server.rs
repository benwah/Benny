use crate::distributed_network::{
    DistributedNetwork, MessagePayload, NetworkMessage, ProtocolError,
};
...
impl NetworkServer {
        ...
        match message.payload {
            ...
            MessagePayload::HebbianData {
                ...
                if self.config.hebbian_learning {
                    self.handle_hebbian_data(layer_id, correlations, learning_rate)
                        .await?;
                }
            }
            MessagePayload::Handshake {
                ...
                self.handle_handshake(network_id, name, layers, capabilities)
                    .await?;
            }
    ...
    async fn handle_forward_data(&self, layer_id: u8, data: Vec<f32>) -> Result<(), ProtocolError> {
        ...
        info!(
            "📥 Received activation for layer {} with {} inputs",
            layer_id,
            inputs.len()
        );
        ...
        info!(
            "⚡ Processed in {:.2}ms, outputs: {:?}",
            processing_time, outputs
        );
    ...
    async fn handle_hebbian_data(
        &self,
        layer_id: u8,
        _correlations: Vec<f32>,
        learning_rate: f32,
    ) -> Result<(), ProtocolError> {
        info!(
            "🧠 Received Hebbian data for layer {} (rate: {})",
            layer_id, learning_rate
        );
...
pub async fn run_daemon(network: NeuralNetwork, config: ServerConfig) -> Result<(), ProtocolError> {
    // Initialize logging
    env_logger::init();
...
impl NetworkClient {
    ...
    pub async fn connect_to_network(
        ...
        info!(
            "🔗 Connecting to neural network at {}:{}",
        ...
        let peer_id = self
            .distributed_network
            .connect_to(target_address, target_port)
            .await?;
    ...
    pub async fn send_forward_data(
        ...
        self.distributed_network
            .send_forward_data(peer_id, layer_id, data)
            .await
    }
    ...
    pub async fn send_hebbian_data(
        ...
        // Use the existing distributed network send method
        self.distributed_network
            .send_hebbian_data(peer_id, layer_id, correlations, learning_rate)
            .await
    }
}
