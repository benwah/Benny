...
fn main() {
    ...
    println!("📊 Average per forward pass: {:?}", single_forward_time / 1000);
    
    // Test 2: Batch Forward Pass Performance
    println!("\n🔬 Test 2: Batch Forward Pass");
    println!("-----------------------------");
    
    let batch_inputs: Vec<Vec<f64>> = training_data.iter()
        .take(100)
    ...
    println!("✅ Batch outputs shape: {} x {}", batch_outputs.len(), batch_outputs[0].len());
    ...
    println!("⏱️  100 individual training steps: {:?}", single_training_time);
    ...
    println!("⏱️  100 samples in batch training: {:?}", batch_training_time);
    ...
    println!("⏱️  10 training steps on large network: {:?}", large_training_time);
    println!("📊 Average per training step: {:?}", large_training_time / 10);
    ...
    for (i, arch) in architectures.iter().enumerate() {
        ...
        let test_target: Vec<f64> = (0..output_size).map(|j| (j as f64 * 0.02).cos().abs()).collect();
    ...
    println!("⚡ Expected speedup: ~{}x on multi-core systems", num_cores.min(4));
