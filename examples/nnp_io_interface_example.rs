...
use neural_network::{
    ExternalSinkConfig, ExternalSourceConfig, InputNode, IoNodeConfig, OutputNode, SecureInputNode,
};
use uuid::Uuid;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        if let Err(e) = output_node_clone
            .process_messages(output_receiver, actuator_sink)
            .await
        {
    ...
    println!(
        "  External Sensor → InputNode → NNP → DistributedNetwork → NNP → OutputNode → External Actuator"
    );
    println!(
        "  
    ┌─────────────┐    NNP     ┌──────────────────┐    NNP     ┌─────────────┐
