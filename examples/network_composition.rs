use neural_network::{NetworkComposer, NeuralNetwork};
...
fn simple_pipeline_example() {
    ...
    // Add to composer
    composer
        .add_network("feature_extractor".to_string(), feature_extractor)
        .unwrap();
    composer
        .add_network("classifier".to_string(), classifier)
        .unwrap();
...
fn fan_out_example() {
    ...
    // Add to composer
    composer
        .add_network("shared".to_string(), shared_processor)
        .unwrap();
    composer
        .add_network("specialist_a".to_string(), specialist_a)
        .unwrap();
    composer
        .add_network("specialist_b".to_string(), specialist_b)
        .unwrap();

    // Connect shared processor to both specialists
    composer
        .connect_networks("shared", "specialist_a", vec![0, 1], vec![0, 1])
        .unwrap();
    composer
        .connect_networks("shared", "specialist_b", vec![2, 3], vec![0, 1])
        .unwrap();
...
fn complex_processing_example() {
    ...
    // Add networks
    composer
        .add_network("input_a".to_string(), input_processor_a)
        .unwrap();
    composer
        .add_network("input_b".to_string(), input_processor_b)
        .unwrap();
    composer
        .add_network("fusion".to_string(), fusion_network)
        .unwrap();
    composer
        .add_network("output".to_string(), output_network)
        .unwrap();

    // Create connections
    composer
        .connect_networks("input_a", "fusion", vec![0, 1], vec![0, 1])
        .unwrap();
    composer
        .connect_networks("input_b", "fusion", vec![0, 1], vec![2, 3])
        .unwrap();
    composer
        .connect_networks("fusion", "output", vec![0, 1, 2], vec![0, 1, 2])
        .unwrap();
...
fn ensemble_example() {
    ...
    // Add networks
    composer
        .add_network("classifier_1".to_string(), classifier_1)
        .unwrap();
    composer
        .add_network("classifier_2".to_string(), classifier_2)
        .unwrap();
    composer
        .add_network("classifier_3".to_string(), classifier_3)
        .unwrap();
    composer
        .add_network("voter".to_string(), voting_network)
        .unwrap();

    // Connect all classifiers to the voting network
    composer
        .connect_networks("classifier_1", "voter", vec![0], vec![0])
        .unwrap();
    composer
        .connect_networks("classifier_2", "voter", vec![0], vec![1])
        .unwrap();
    composer
        .connect_networks("classifier_3", "voter", vec![0], vec![2])
        .unwrap();
    ...
    let target = vec![1.0];
    let error1 = composer
        .train_network("classifier_1", &test_input, &target)
        .unwrap();
    let error2 = composer
        .train_network("classifier_2", &test_input, &target)
        .unwrap();
    let error3 = composer
        .train_network("classifier_3", &test_input, &target)
        .unwrap();

    println!(
        "Training errors - C1: {:.4}, C2: {:.4}, C3: {:.4}",
